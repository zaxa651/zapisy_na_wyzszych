    ...
    return (
      <View style={styles.noLecturersBox}>
        ...
        <Text style={styles.noLecturersText}>
          Скорее всего, RLS-политика на таблице{' '}
          <Text style={{ fontFamily: 'monospace' }}>profiles</Text> не разрешает
          администратору читать все профили.{'\n\n'}
          Выполни этот SQL в Supabase → SQL Editor:
        </Text>
        <View style={styles.sqlBox}>
          <Text style={styles.sqlCode}>{RLS_SQL}</Text>
        </View>
        <Text style={styles.noLecturersText}>После выполнения обнови список ↓</Text>
...
const EditModal = ({ course, lecturers, onSave, onClose, onRefreshLecturers }: EditModalProps) => {
    ...
    <Modal visible={!!course} animationType="slide">
      <SafeAreaView style={styles.modalContainer}>
        <ScrollView style={{ padding: 20 }}>
          <Text style={styles.modalTitle}>Редактирование: {course?.title}</Text>
...
export default function AdminPanel() {
  ...
  const handleSave = useCallback(async (lecturerId: string | null, slots: string[]) => {
...
const styles = StyleSheet.create({
  ...
  courseMiniCard:   { backgroundColor: '#fff', padding: 15, borderRadius: 12, marginBottom: 10, flexDirection: 'row', alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 5, elevation: 2 },
  ...
  noLecturersBox:   { backgroundColor: '#FFF3E0', padding: 20, borderRadius: 16, marginBottom: 20 },
  noLecturersTitle: { fontSize: 18, fontWeight: 'bold', color: '#E65100', marginBottom: 12, textAlign: 'center' },
  noLecturersText:  { color: '#E65100', fontSize: 15, lineHeight: 22, marginBottom: 15 },
  sqlBox:           { backgroundColor: '#1C1C1E', padding: 14, borderRadius: 12, marginVertical: 12 },
  sqlCode:          { fontFamily: 'monospace', color: '#34C759', fontSize: 13, lineHeight: 20 },
  refreshBtn:       { backgroundColor: '#007AFF', paddingVertical: 14, borderRadius: 12, alignItems: 'center' },
  refreshBtnText:   { color: '#fff', fontWeight: 'bold', fontSize: 16 },

  inputGroup:       { flexDirection: 'row', alignItems: 'center', marginBottom: 10 },
  input:            { flex: 1, backgroundColor: '#fff', height: 50, paddingHorizontal: 15, borderRadius: 10, borderWidth: 1, borderColor: '#D1D1D6', fontSize: 16 },
  addBtn:           { backgroundColor: '#007AFF', width: 50, height: 50, justifyContent: 'center', alignItems: 'center', borderRadius: 10, marginLeft: 10 },
  addBtnText:       { color: '#fff', fontSize: 28, fontWeight: '300' },

  slotsContainer:   { flexDirection: 'row', flexWrap: 'wrap', marginTop: 10 },
  slotBadge:        { backgroundColor: '#34C759', borderRadius: 8, marginRight: 8, marginBottom: 8, flexDirection: 'row', alignItems: 'center', paddingLeft: 12 },
  slotText:         { color: '#fff', fontWeight: 'bold', fontSize: 14 },
  deleteSlotBtn:    { padding: 10, marginLeft: 5, backgroundColor: 'rgba(0,0,0,0.1)', borderTopRightRadius: 8, borderBottomRightRadius: 8 },
